%loads the data that has already been validated, and allows it to be looked
%at again

fileNum=2;


finalDataStorage='F:\RibbonAnalysisDataSets\Final';
folderName=strcat(num2str(fileNum),'Final');
fullFolderName=strcat(finalDataStorage,'\',folderName);
matlabPath='C:\Users\togha\Documents\MATLAB\OghalaiLabRibbonAnalysis';
movefile(fullFolderName,matlabPath);
addpath(folderName);

finalDataName=strcat(num2str(fileNum),'FinalMat.mat');
load(finalDataName);

newCenters=saveInOrigForm{2, 1}(:,1:3);
mu=saveInOrigForm{2, 1}(:,4);
ribbon=saveInOrigForm{2, 9};
voxel=saveInOrigForm{2, 4};

movefile(folderName,finalDataStorage);



dataSettings=readtable('DataSettings4.xlsx');

DSLocation1='F:\RibbonAnalysisDataSets\tiffData';
[allData,~,dimensions,~]=loadTiff(fileNum,'RawData',DSLocation1);

DSLocation2='F:\RibbonAnalysisDataSets\FilteredData';
[allFiltered,voxel,dimensions,minMax]=loadTiff(fileNum,'FilteredData',DSLocation2);
...
minGroup=5;
threshMat=table2array(dataSettings(fileNum,2:3));
%threshMat=[.32,.29];
groupSize=[3,3];
